...
//						$content = strip_tags(_get_texthtmlnew($config_webserver_ip, stripslashes($content), $item));
			...
			case "image/jpeg":
			case "image/png":
				$content = '<p style="text-align: center;"><img src="'.$config_webserver_ip.'/thumbnail/'.$attributes["OBJ_ID"].'/0/100" border="0" /></p>';
				$qtip0 = msg('SHOW_HERE');
...
//						$mimeType = $attributes["DOC_MIME_TYPE"];
				...
				break;
			default:
				$content = '<a href="'.$config_webserver_ip.'/get/'.$attributes["OBJ_ID"].'" title="'.$attributes["OBJ_NAME"].'">'.$attributes["OBJ_NAME"].'</a>';
...
function getCustomImage($steam){
 ...
 if (in_array("OBJ_ICON", $attributes)){
	 ...
	 print ('<img src="'.$config_webserver_ip.'/thumbnail/'.$imageId.'/0/100" border="0" height="100%" />'); //TODO: Not working locally
