LarsDesktopGrid = function(region) {
    ...
    var store = new Ext.data.GroupingStore({
			proxy: new Ext.data.HttpProxy({
                url: 'data/', //url to server side script
                method: 'POST',
	            timeout: 120000
            }),
			...
			if (action.match("file")){
				switch(rec.data.action2) {
					...
					case 'editPage':
			    		...
			    		break;
					case 'pdf' :
						node2.attributes = {lars_ref : "get/"+node2.id, iconCls: node2.attributes.iconCls};
						...
						switch(action) {
							...
							default:
					        	Ext.Msg.confirm(
									...
									function(btn){
										if (btn == 'yes'){
											LarsGridConfig.downloadFile("get/"+rec.data.id);
										}
								}, this);
								break;
						}
